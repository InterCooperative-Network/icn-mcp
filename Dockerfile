...
# Force npm install to ensure TypeScript is available
RUN npm install
...
RUN ls -la node_modules/.bin/tsc && echo "TypeScript binary found" || echo "TypeScript binary NOT found"

# Build the server workspace using installed TypeScript
RUN npm run build
...
# copy the built server artifacts from the correct location
