...
describe('Workflow Engine Integration', () => {
    ...
    it('should create checkpoints with idempotency', async () => {
        const workflowId = await workflowEngine.startWorkflow('test-workflow', {}, 'test-agent');
        // Create checkpoint
        const checkpoint1 = await workflowEngine.createCheckpoint(workflowId, 'step1', { progress: 50 }, 'halfway done', 'req-123');
        ...
        const checkpoint2 = await workflowEngine.createCheckpoint(workflowId, 'step1', { progress: 60 }, // Different data
        'different note', 'req-123' // Same request ID
        );
