...
import { icnStartWorkflow, icnGetNextStep, icnCheckpoint, icnListWorkflowTemplates, icnWorkflow } from '../src/tools/icn_workflow.js';
...
describe('Error Handling and Edge Cases', () => {
    ...
    describe('Core Tools Error Handling', () => {
        ...
        describe('icnGetInvariants', () => {
            it('should return core invariants even when files are missing', async () => {
                ...
                const invariantTexts = result.invariants.map(inv => typeof inv === 'string' ? inv : inv.description || inv.title || inv.statement || JSON.stringify(inv)).join(' ').toLowerCase();
        ...
        describe('icnCheckPolicy', () => {
            ...
            it('should handle unknown actor gracefully', async () => {
                ...
                if (!result.allow) {
                    expect(result.reasons.some(reason => reason.toLowerCase().includes('unknown') ||
                        reason.toLowerCase().includes('actor'))).toBe(true);
                }
            });
        });
        ...
        describe('icnSuggestApproach', () => {
            it('should handle empty task description', async () => {
                try {
                    ...
                    expect(error).toBeInstanceOf(Error);
                    expect(error.message).toContain('task_description');
                }
            });
            ...
            it('should handle excessive constraints', async () => {
                const manyConstraints = Array.from({ length: 50 }, (_, i) => `Constraint number ${i + 1}: very specific requirement`);
                const result = await icnSuggestApproach({
    ...
    describe('Acceptance Tests for ICN Invariants', () => {
        ...
        it('should ensure tools validate against ICN coordination credits invariants', async () => {
            ...
            const hasInvariantCheck = result.plan.steps.some(step => step.tool === 'icn_check_invariants' ||
        ...
        it('should ensure invariant checks catch violations of ICN principles', async () => {
            ...
            const invariantTexts = invariantResult.invariants.map(inv => typeof inv === 'string' ? inv : JSON.stringify(inv)).join(' ').toLowerCase();
