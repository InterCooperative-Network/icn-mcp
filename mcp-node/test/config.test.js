...
test('mcp-node config provides correct paths', () => {
    ...
    try {
        ...
        // Restore environment
        if (originalDocRoot !== undefined)
            process.env.DOCS_ROOT = originalDocRoot;
        else
            delete process.env.DOCS_ROOT;
        if (originalPolicyPath !== undefined)
            process.env.POLICY_RULES_PATH = originalPolicyPath;
        else
            delete process.env.POLICY_RULES_PATH;
        if (originalCodeownersPath !== undefined)
            process.env.CODEOWNERS_PATH = originalCodeownersPath;
        else
