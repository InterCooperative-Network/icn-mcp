...
import { ICN_MCP_LOG_DIR } from './config.js';
...
class ICNMCPServer {
  ...
  private async readResource(uri: string) {
    try {
      ...
      if (uri === 'icn://logs/recent') {
        const content = await this.readRecentLogs();
        return [{
          uri,
          mimeType: 'text/plain',
          text: content
  ...
  private async readRecentLogs(maxKilobytes: number = 10): Promise<string> {
    try {
      const logDir = ICN_MCP_LOG_DIR;
