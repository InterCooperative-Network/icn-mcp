...
export function initPolicyWatcher(onReload) {
...
export function checkPolicy(input) {
    ...
    if (rules.path_caps && Object.prototype.hasOwnProperty.call(rules.path_caps, input.actor)) {
        ...
        for (const p of input.changedPaths) {
            ...
            if (!allowed)
                reasons.push(`path ${p} not allowed for actor ${input.actor}`);
        }
    }
    ...
    if (rules.reviews_required) {
        for (const rule of rules.reviews_required) {
            const affectedPaths = input.changedPaths.filter(path => rule.paths.some(pattern => matchGlob(pattern, path)));
